...
private:
    ...
    typedef struct MEMORY_HEAD
    {
        ...
        byte_pointer    item;      // 分配出去的内存, 每次分配出去都指向下一个成员, 直到越界后就从链表中取下一个节点
    }*PMEMORY_HEAD;
...
public:
    CMemoryPool() :_Mem(nullptr), _Al(), _Now(nullptr)
    {

    }
    explicit CMemoryPool(size_t size) :_Mem(nullptr), _Al(), _Now(nullptr)
    {
        init(size);
    }
    ...
    CMemoryPool(CMemoryPool&& other) noexcept :_Mem(nullptr), _Al(), _Now(nullptr)
    {
    ...
    inline void Release()
    {
        ...
        while (node)
        {
            ...
            _Al.deallocate(reinterpret_cast<byte_pointer>(node), node->size);
            node = next;
        }
        _Mem = nullptr;
        _Now = nullptr;
...
public:
    ...
    inline void swap(CMemoryPool& other)
    {
    ...
    inline pointer realloc_arr(pointer pArr, size_t _OldSize, size_t _Size)
    {
        ...
        byte_pointer pArrStart = reinterpret_cast<byte_pointer>(pArr);
        byte_pointer pArrEnd = pArrStart + sizeof(value_type) * _OldSize;
        bool isFree = true;
        ...
        const size_t newSize = (std::min)(_OldSize, _Size);
    ...
    inline bool free_arr(pointer p, size_t _Size)
    {
        ...
        byte_pointer pArrStart = reinterpret_cast<byte_pointer>(p);
        byte_pointer pArrEnd = pArrStart + sizeof(value_type) * _Size;
        byte_pointer pEnd = reinterpret_cast<byte_pointer>(pHead) + pHead->size;
        if (pArrEnd == pHead->item || pArrEnd == pEnd)
        {
            // 被释放的这个数组是连着下一个分配的内存, 直接把下一个分配的内存指向回收的这个数组
    ...
    inline void dump() const
    {
        ...
        while (pHead)
        {
            ...
            printf("%03d: 内存块首地址 0x%p, 内存块起始分配地址: 0x%p, 内存块尺寸: %u, 已分配 %d 个成员, 当前回收的内存链表 = 0x%p\n",
                   index++, pHead, pAllocStart, (uint32_t)pHead->size, count, pHead->list);
            pHead = pHead->next;
        }
    }
...
private:
    ...
    inline bool is_head(PMEMORY_HEAD pHead, const void* p) const
    {
        ...
        //byte_pointer pEnd = pHead->arr ? pHead->arr : (pStart + pHead->size - sizeof(MEMORY_HEAD));
    ...
    inline pointer alloc_for_list(PMEMORY_HEAD pHead, size_t _Size, bool isClear)
    {
        ...
        byte_pointer pNodeStart = reinterpret_cast<byte_pointer>(pHead->list);
        byte_pointer pNodeEnd = pNodeStart + (sizeof(value_type) * _Size);
    ...
    inline PMEMORY_HEAD malloc_head(size_t count)
    {
        ...
        byte_pointer pStart = _Al.allocate(newSize);
        byte_pointer pEnd = pStart + newSize;
    ...
    // pFreeEnd = 释放的内存结束地址
    // _FreeCount = 释放的内存成员数
    inline void combine_free_pointer(PMEMORY_HEAD pHead, byte_pointer pFreeStart, byte_pointer pFreeEnd, size_t _FreeCount)
    {
        ...
        byte_pointer pItemStart = reinterpret_cast<byte_pointer>(pHead) + sizeof(MEMORY_HEAD);
        byte_pointer pItemEnd = pItemStart + pHead->size - sizeof(MEMORY_HEAD);
        ...
        byte_pointer pNodeStart = reinterpret_cast<byte_pointer>(pHead->list);
        byte_pointer pNodeEnd = pNodeStart + (sizeof(value_type) * count);
    ...
    void combine_pointer(PMEMORY_HEAD pHead, PLIST_NODE pNode2)
    {
        ...
        byte_pointer pNodeStart1 = reinterpret_cast<byte_pointer>(pHead->list);
        byte_pointer pNodeEnd1 = pNodeStart1 + (sizeof(value_type) * count1);

        byte_pointer pNodeStart2 = reinterpret_cast<byte_pointer>(pNode2);
        byte_pointer pNodeEnd2 = pNodeStart2 + (sizeof(value_type) * count2);
        pNextNode = pNext2;
...
//    int GetItemStartEnd(PMEMORY_HEAD pHead, byte_pointer& pStart, byte_pointer& pEnd)
//    {
//        pStart = (byte_pointer)pHead;
...
//    {
//        byte_pointer pStart = (byte_pointer)pHead;
//        byte_pointer pEnd = pStart + pHead->size;
//        byte_pointer pFirst = pStart + sizeof(MEMORY_HEAD);
//
...
//    int IsItemAddress(LPCVOID ptr, byte_pointer begin, byte_pointer end)
//    {
//        byte_pointer p = (byte_pointer)ptr;
//        if (p < begin || p >= end)
//            return -1;
//
...
//            byte_pointer pNodeStart = reinterpret_cast<byte_pointer>(node);
//            byte_pointer pNodeEnd = pNodeStart + (sizeof(value_type) * count);
...
//        // 如果下一个分配的地址为空, 那就是指向这块内存的结束地址
//        byte_pointer pStart = reinterpret_cast<byte_pointer>(pHead) + sizeof(MEMORY_HEAD);
//        byte_pointer pEnd = pHead->item ? pHead->item : (pStart + pHead->size - sizeof(MEMORY_HEAD));
//        byte_pointer pFirst = pStart;
//
//        byte_pointer p = (byte_pointer)ptr;
//        const int index = IsItemAddress(ptr, pFirst, pEnd);
